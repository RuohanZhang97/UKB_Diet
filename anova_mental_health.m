...
for num_group = 3:5
    count = count + 1;
    anova_mental_food_group_results(count).num_group = num_group;
    str = strcat('clusters_',num2str(num_group));
    temp_group_ind = food_category_clustering.(str);
    temp_group_ind = temp_group_ind(ind_food,:);
    anova_mental_food_group_results(count).final_food_group_ind = temp_group_ind;
    anova_mental_food_group_results(count).final_food_SubID = mental_cov_food_SubID;
    anova_mental_food_group_results(count).NumSub = length(mental_cov_food_SubID);
    for j = 1:length(mental_names)
        ...
        anova_mental_food_group_results(count).(strcat(mental_names{j,1},'_regressed_score')) = regressed_score;
        [p,tbl] = anova1(regressed_score, temp_group_ind, 'off');
        Fstat = tbl{2,5};
        anova_mental_food_group_results(count).(strcat(mental_names{j,1},'_Fstat')) = Fstat;
        anova_mental_food_group_results(count).(strcat(mental_names{j,1},'_pVal')) = p;
    end
end
...
save(savefile,'anova_mental_food_group_results','mental_food_group_scores');
